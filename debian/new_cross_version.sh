...
case "$arch" in
    arm64)
	pkg_all=libgcc-16-dev-arm64-cross;;
    ppc64)
	pkg_all=libgcc-16-dev-ppc64-cross;;
    mipsr6el)
	pkg_all=libgcc-16-dev-mipsr6el-cross;;
    *)
	echo >&2 "usage: $0 <architecture>"
	exit 1
esac
case "$(dpkg-architecture -qDEB_HOST_ARCH)" in
    arm64)
	pkg_any=gcc-16-arm-linux-gnueabihf-base;;
    ppc64)
	pkg_any=gcc-16-powerpc64le-linux-gnu-base;;
    ppc64el)
	pkg_any=gcc-16-powerpc-linux-gnu-base;;
    s390x)
	pkg_any=gcc-16-powerpc64-linux-gnu-base;;
    amd64|i386|x32)
	case "$arch" in
	    arm64)
		pkg_any=gcc-16-aarch64-linux-gnu-base;;
	    ppc64)
		pkg_any=gcc-16-powerpc64-linux-gnu-base;;
	    mipsr6el)
		pkg_any=gcc-16-mipsisa32r6el-linux-gnu-base;;
	esac;;
esac

v_deb_gcc=$(apt-cache policy gcc-16-source | awk '/^ \*\*\*/ {print $2}')
